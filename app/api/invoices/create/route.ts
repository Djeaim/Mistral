...
export async function POST(req: NextRequest) {
  ...
  const { customer_id, issue_date, due_date, currency = 'EUR', vat_rate, notes, lines = [], related_quote_id } = body || {};
  if (!customer_id || !issue_date || !due_date || !Array.isArray(lines) || lines.length === 0) return NextResponse.json({ error: 'Missing fields' }, { status: 400 });
  ...
  const path = `documents/${auth.user.id}/invoices/${number}.pdf`;
